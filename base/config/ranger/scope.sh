...
handle_extension() {
	...
	js | jsx | ts | tsx | mjs | cjs)
		...
		env HIGHLIGHT_OPTIONS="${HIGHLIGHT_OPTIONS}" highlight \
			--out-format=ansi --force --syntax=javascript \
			-- "${FILE_PATH}" && exit 5
	...
	## MessagePack
	msgpack | mpack | msgp)
		if command -v msgpack2json &>/dev/null; then
			msgpack2json "${FILE_PATH}" | jq --color-output . && exit 5
		fi
		python3 -c "
import sys
try:
...
handle_mime() {
	...
	*sqlite3)
	...
	image/*)
		...
		# exiftool "${FILE_PATH}" && exit 5
		viu --width "${PV_WIDTH}" -- "${FILE_PATH}" && exit 5
		exit 1
		;;
